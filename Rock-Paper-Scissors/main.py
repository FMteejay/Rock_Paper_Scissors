...
def play():
    user = input("What's your choice? 'r' fir rock, 'p' for paper, 's' for scissors\n")
    user = user.lower()
...
def is_win(player, opponent):
    #return true if thr player beats the opponent
    # winning conditions: r > s, s > p, p > r
    if (player == 'r' and opponent == 's') or (player == 's' or (player == 'p' and opponent == 'r')):
        return True
    return False

def play_best_of(n):
    # play against the computer until someone wins best of n games
    # to win, you must win cell(n/2) games (ie 2/3, 3/4, 4/7)
    player_wins = 0
    computer_wins = 0
